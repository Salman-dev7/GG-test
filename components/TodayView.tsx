...
const TodayView: React.FC<TodayViewProps> = ({ habits, checkins, onToggle, onAddClick, accentColor }) => {
  ...
  return (
    <div className="h-full flex flex-col px-4 space-y-4">
      ...
      {isCalendarOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 p-6 animate-in fade-in duration-200">
          <div className="w-full max-w-[220px] rounded-[24px] glass p-3.5 animate-in zoom-in-95 duration-300 flex flex-col space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[6px] uppercase tracking-[0.2em] font-black opacity-40">Jump to date</p>
                <h2 className="text-xs font-extrabold">Calendar</h2>
              </div>
              <button
                onClick={() => setIsCalendarOpen(false)}
                className="w-6 h-6 flex items-center justify-center glass rounded-full opacity-60"
              >
                <X size={10} />
              </button>
            </div>
            <input
              ...
              className="w-full rounded-lg px-2 py-1.5 text-[10px] font-bold bg-white/10 text-current outline-none border border-white/10"
            />
            <button
              className="w-full rounded-lg py-1.5 text-[7px] font-black uppercase tracking-widest glass active:scale-95 transition-transform"
              style={{ color: accentColor }}
              onClick={() => {
